if [ "$INSTALL_BOOTLOADER" = "grub-pc" ] && [ "$INSTALL_CHROOT_ONLY" = "false" ]; then
	...
	if $? -ne 0; then log "Failed to mount efivars to /sys/firmware/efi/efivars. EFI bootloader installation may fail."; umount_efi; sleep 60; exit 1; fi

	if [ "$FORMART_EFI_PARTITION" = "true" ]; then
		log "not supported rn LOL"; umount_efi; exit 1 # TODO xD
	...
	if [ "$MOUNT_REAL_ROOT" = "true" ]; then
		...
		if $? -ne 0; then log "Failed to mount real root partition $FSTAB_ROOT_DEVICE with -o '$FSTAB_ROOT_MOUNT_OPTIONS' to /mnt. EFI bootloader installation may fail."; sleep 60; umount_efi; exit 1; fi
	...
	if [ "$MOUNT_REAL_BOOT" = "true" ]; then
		...
		if $? -ne 0; then log "Failed to mount real boot partition $FSTAB_BOOT_DEVICE to /boot/efi. EFI bootloader installation may fail."; sleep 60; umount_efi; exit 1; fi
	fi
	...
	if [ $? -ne 0 ]; then
		...
		if [ $? -ne 0 ]; then log "User aborted EFI bootloader installation. Please fix the issue manually."; sleep 5; umount_efi; exit 1; fi
		sleep 15
		...
		if [ $? -ne 0 ]; then log "grub-install with --removable also failed. EFI bootloader installation has likely failed. Please fix the issue manually."; sleep 5; umount_efi; exit 1; fi
	fi
